...
## Behaviour
- Owns all state: `params` (DashboardParams) and `fetchTrigger` (number).
- `params` defaults: fromDate = today, toDate = today, stockExchange = "DSE".
- `useDashboardData` only fires when `fetchTrigger > 0` (no auto-fetch on mount).
- onFetch: increment fetchTrigger → triggers the hook.
- Show amber warning banner when `error` contains "Market data unavailable".
- Show red error banner + Retry button when all brokers failed.
...
Note: if TypeScript complains about `activeParams`, simplify by removing the `useMemo` and always passing `params` — the hook re-runs on every `fetchTrigger` increment naturally.
