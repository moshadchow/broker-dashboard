  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (marketResp?.market) {
        ...
        const marketValue = Number(d.values);
        if (marketValue > 0) {
          brokerRows.forEach(r => { r.valueSharePct = (Number(r.totalValue) / marketValue) * 100; });
          aggregateRow.valueSharePct = (Number(aggregateRow.totalValue) / marketValue) * 100;
        }
      } else {
